...
def parse_markdown_to_jsons(filename):
    ...
    # Find all JSON blocks under any results subdirectory
    # Looking for ### results\<set>\... followed by ```json ... ```
    pattern = r'### results\\[^\n]+\n+```json\n(.*?)\n```'
...
def fmt(val, spec="{:.1f}"):
    """Format a metric cell, tolerating missing strategy/concurrency combos."""
    return "-" if val is None else spec.format(val)
...
def main():
    ...
    for s in skews:
        ...
        print(f"| {s} | {fmt(occ)} | {fmt(pess)} | {fmt(ssi)} |")
    ...
    for c in concs:
        ...
        print(f"| {c} | {fmt(occ)} | {fmt(pess)} | {fmt(ssi)} |")
        
    print("\n=== Latency C=250, Theta=0.8 ===")
    print("(Note: db_latency is the full in-transaction window, comparable across")
    print(" strategies; cc_wait_latency is the lock-acquisition component and is")
    print(" only meaningful for PESSIMISTIC. Results generated before the")
    print(" uniform taxonomy fix used per-strategy definitions.)")
